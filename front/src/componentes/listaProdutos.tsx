...
export default class ListaProdutos extends Component<Props, State> {
    ...
    handleExcluirProduto = async (nome: string) => {
        if (window.confirm('Tem certeza que deseja excluir este produto?')) {
            try {
                await produtosApi.excluir(nome);
    ...
    render() {
        const { produtos, loading, error } = this.state;
                                ...
                                produtos.map((produto) => (
                                    <tr key={produto.id} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <button
                                                onClick={() => this.props.onEditarProduto(produto)}
                                                className="text-indigo-600 hover:text-indigo-900 mr-4"
                                            >
                                                Editar
                                            </button>
                                            <button
                                                onClick={() => this.handleExcluirProduto(produto.nome)}
